...
static std::string keyIdAndKeyPairsToJSON(media::KeyIdAndKeyPairs *g_keys) {
    ...
    {
    #if PRINT_HEX
      printf("Update key pairs:\n");
      hex_print(it->first.c_str(), it->first.length());
      hex_print(it->second.c_str(), it->second.length());
    #endif
...
void CMediaKeySession::Update(
  ...
  media::ExtractKeysFromJWKSet(key_string, &m_keyPair, 0);
  ...
  keys_updated = keyIdAndKeyPairsToJSON(&m_keyPair);
...
CDMi_RESULT CMediaKeySession::Decrypt(
  ...
  if(m_keyPair.size() != 1) {
        cout << "FIXME: We support only one key at the moment. Number keys: " << m_keyPair.size()<< endl;
    }

  if ( (m_keyPair[0].second).size() != kDecryptionKeySize) {
    cout << "ERROR: Wrong key size" << endl;
    goto fail;
  }

  key = (m_keyPair[0].second).data();
